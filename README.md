...
- **Status** — **Neo v2.2 UI is feature-complete.** Platform **Phase 8A** (v2.3.0-dev) generalized the remaining airfoil-shaped **identity** paths: `Experiment.case_id` / `geometry_key` / `validate` are now template-driven, and the Internal Flow bridge is removed (see `docs/PLATFORM_ARCHITECTURE.md` §17). **Phase 8B** made the **RESULT** half generic: a template-attached `CaseResult` serializes through template-defined metrics (name / value / unit / status) with generic `to_json_dict` / `from_json_dict` round-trips, legacy result JSON stays byte-identical, and the generic output-column contract is declared on the templates (§18). The **write side** — generic Excel, ledger, analytics (Phases 8C–8H) — remains future work.
