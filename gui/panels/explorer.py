...
class ExplorerPanel(QWidget):
    ...
    def rebuild(self) -> None:
        ...
        if st.cfg.fluent.baseline_case:
            add(proj, f"⬢ Baseline case — {Path(st.cfg.fluent.baseline_case).name}",
                Path(st.cfg.fluent.baseline_case))
        ...
        if cases_dir.exists():
            for case in sorted(cases_dir.iterdir()):
                if not case.is_dir():
                    continue
                node = add(runs, f"📁 {case.name}", case,
                           row_by_case.get(case.name, -1))
